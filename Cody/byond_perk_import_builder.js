...
fs.readFile(process.argv[2], 'utf8', (err, data) => {
    ...
    for (var y = 0; y < objArr.length; y++) {
        ...
        output += 'var/obj/NewPerk'+y+' = new/obj/jutsu/perk\r\n'+
			'NewPerk'+y+':jutsu_type="'+objArr[y].jutsu_type+'"\r\n'+
            'NewPerk'+y+':name="' + pName + '"\r\n' +
            'NewPerk'+y+':point_cost=' + objArr[y].point + '\r\n' +
            'NewPerk'+y+':perk_type="'+objArr[y].perk_type+'"\r\n' +
            'NewPerk'+y+':control_req='+objArr[y].control+'\r\n' +
            'NewPerk'+y+':strength_req='+objArr[y].strength+'\r\n' +
            'NewPerk'+y+':agility_req='+objArr[y].agility+'\r\n' +
            'NewPerk'+y+':stamina_req='+objArr[y].stamina+'\r\n' +
            'NewPerk'+y+':speed_req='+objArr[y].speed+'\r\n' +
            'NewPerk'+y+':endurance_req='+objArr[y].endurance+'\r\n' +
            'NewPerk'+y+':icon=\''+iconFile+'\'\r\n' +
            'NewPerk'+y+':desc={"'+desc+'"}\r\n';
			
			if(objArr[y].prereqArr.toString().length>1)
				output+='NewPerk'+y+':prerequisites=list('+objArr[y].prereqArr.toString().replace(/,$/,"").replace(/^|$/g,"\"").replace(/,/g,"\",\"")+')\r\n';
            
			output +='Archive_Perk_Nosave(NewPerk'+y+')\r\n';
	...
	fs.writeFile("perk_archive_to_import.txt",output, (err) => {
	  if (err) throw err;
