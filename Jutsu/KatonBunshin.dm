mob/KatonBunshin/MouseDrag()
	if(src.displaykey==usr.key)
		mouse_drag_pointer=icon(src.icon,src.icon_state,src.dir)
mob/KatonBunshin/New()
	..()
	mouse_drag_pointer=icon(src.icon,src.icon_state,src.dir)


mob/KatonBunshin/MouseDrop(o_o,s_l,o_l) //make it so if an object has been dragged to someplace...
	..()
	if(src.displaykey==usr.key)
			...
			switch(input("Choose Option","",text) in Choices)
				...
				if("Destroy Bunshins")
					if(usr.MindTransfer)
						...
						usr.client.perspective=EYE_PERSPECTIVE
					usr.bunshinList -= src
					for(var/mob/KatonBunshin/A)
						if(A==src) continue
						if(A.displaykey==usr.key)
							usr.bunshinList -= A
							A.overlays -= A.overlays
							spawn() flick('Fire.dmi',A)
							del(A)
					del(src)
				if("Destroy Bunshin")
					if(usr.MindTransfer)
				...
				if("Attack Target")
					...
					for(var/mob/M in oviewers(12)) if(Choice==M)
						spawn while(function==3)
							...
							BunshinAttack()
obj/Ninjutsu/KatonBunshin
	icon='Skillcard.dmi'
	icon_state="KatonBunshin"
	Click()
		if(src in usr.contents)
			...
			if(!usr.move)
				return
			for(var/mob/KatonBunshin/Z)
				if(Z.displaykey==usr.key)
					bunshins+=1
			if(bunshins<skill)
				if(prob(10/skill/skill/skill))
					usr<<output("<font size = -3>Your understanding of the Katon Bunshin no Jutsu has increased.","outputic.output")
					skill+=1
					Update()
				usr.Jutsu_Gain()
				usr.MaxNinjutsu += (1 *0.001)
				usr.Chakra-=20
				var/mob/A=new/mob/KatonBunshin
				usr.bunshinList+=A
				src.Active=1
				flick('Fire.dmi',A)
				A.Health=1
