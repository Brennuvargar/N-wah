		...
		if(src in usr.contents)
			...
			if(bunshins<skill)
				if(prob(10/skill/skill/skill))
					usr<<output("Your understanding of the Raiton Bunshin no Jutsu has increased.","outputic.output")
					skill+=1
					Update()
				usr.Jutsu_Gain()
				...
				usr.bunshinList+=A
				src.Active=1
				viewers(16)<<output("<font color=[usr.SayFont]>[usr] yells<font color = white>: Raiton Bunshin no Jutsu!","outputic.output")
				flick('Spark.dmi',A)
				A.Health=1
				A.MaxHealth=1
