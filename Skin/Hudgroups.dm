...
HudGroup
	var
		...
		// HudGroups can be nested within each other
		list/children
		HudGroup/parent
	...
	proc
		...
		add(x, y, icon_state = null, text = null, width = null, height = null, value = null, layer = null, icon = null, maptext = null, maptext_width = null, maptext_height = null)
			...
			// if it's a HudObject, add that object
			else if(istype(x, /HudObject))
			...
			// if it's a child group, add that to the list of children
			else if(istype(x, /HudGroup))

				if(!children)
					children = list()

				var/HudGroup/child = x
				...
				// make it inherit its new parent's visibility
				if(visible)
					child.show()
				else
					child.hide()

				// make it inherit its new parent's position
				child.pos(null, null, sx, sy)
		...
		remove(argument = -1)
			...
			else if(istype(argument, /HudGroup))

				var/HudGroup/child = argument
		...
		pos(x, y)
			...
			for(var/HudObject/h in objects)
				h.pos(null, null, base_x, base_y)

			if(children)
				for(var/HudGroup/g in children)
					g.pos(null, null)
		...
		hide()
			...
			if(children)
				for(var/HudGroup/h in children)
					h.hide()
